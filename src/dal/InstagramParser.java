		...
		for (JsonElement j : jsonPictures) {
			...
			if (caption.isJsonObject()) {
				if (jsonPicture.get("type").getAsString().equals("image")) {
					...
					int time = caption.getAsJsonObject().get("created_time")
							.getAsInt();
					...
					JsonElement imageStd = images.getAsJsonObject().get(
							"standard_resolution");
					String urlStd = imageStd.getAsJsonObject().get("url")
							.getAsString();
					picture.setUrlStd(urlStd);

					// Gets url to Thumbnail Resolution Picture
					JsonElement imageThumb = images.getAsJsonObject().get(
							"thumbnail");
					String urlThumb = imageThumb.getAsJsonObject().get("url")
							.getAsString();
