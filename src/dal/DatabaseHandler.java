...
public class DatabaseHandler {
	...
	/**
	 * Method to add and save a picture to DB. Checks for duplicate pictures and
	 * saves changes to the relevant picture to DB.
	 * 
	 * @param pic
	 */
	public static void addPictureToDB(PictureData pic) {
		...
		try {
			...
			// Checks if a hashtag, connected to the picture, is already saved
			// in the DB
			...
			for (int i = 0; i < hashtagList.size(); i++) {
				resultHash = DatabaseHandler
						.returnHashtagIfAlreadyExists(hashtagList.get(i));
			...
			// Checks if a picture with the same ID is in DB, and updates the
			// hashtags if exists
	...
	/**
	 * Checks if picture with given ID is already in DB, and returns this
	 * picture as List
	 * 
	 * @param pic
	...
	public static List<PictureData> returnPictureIfAlreadyExists(PictureData pic) {
		...
		try {
			...
			result = session.createQuery(
					"from PictureData where id=\'" + pic.getId() + "\'").list();
			tx.commit();
		} catch (HibernateException e) {
			if (tx != null)
				tx.rollback();
	...
	public static List<PictureData> listHundredNewestPicturesFromDB() {
		...
		try {
			...
			result = session
					.createQuery(
							"from PictureData pd where pd.removeFlag=0 order by pd.createdTime desc")
	...
	public static List<Hashtag> returnHashtagIfAlreadyExists(Hashtag h) {
		...
		try {
			...
			result = session.createQuery(
					"select distinct h from Hashtag h left join fetch h.pictures where hashtag=\'"
							+ h.getHashtag() + "\'").list();
	...
	public static void removePictureDataFromDB(String id) {
		...
		try {
			...
			Query q = session.createQuery("DELETE FROM PictureData WHERE id=\'"
					+ id + "\'");
			q.executeUpdate();
	...
	public static void removeHashtagFromDB(String hashName) {
		...
		try {
			...
			String s = "DELETE FROM Hashtag WHERE hashtag=\'"
	...
	/**
	 * Deletes pictures without any hashtags from DB, given that they are not
	 * flagged.
	 */
	public static void removePicturesWithoutHashTagFromDB() {
		...
		try {
			...
			String s = "DELETE FROM PictureData " + "WHERE removeFlag = (0) "
					+ "AND NOT EXISTS (" + "SELECT * " + "FROM Hash_Pics "
